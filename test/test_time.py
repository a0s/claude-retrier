...
class TestAbsoluteTimes(unittest.TestCase):
    def test_same_day_reset(self):
        ...
        got = cr.parse_reset("You've hit your session limit · resets 1:30pm (Europe/Warsaw)", now)
    ...
    def test_reset_after_midnight_rolls_to_tomorrow(self):
        ...
        got = cr.parse_reset("You've hit your session limit · resets 3:20am (Europe/Warsaw)", now)
    ...
    def test_banner_timezone_wins_over_host_timezone(self):
        ...
        got = cr.parse_reset("You've hit your limit · resets 8pm (Asia/Tokyo)", now)
        self.assertAlmostEqual(hours(got), 10.283, places=2)

        # Same instant, but the host sits in New York. The answer must not move.
        got_ny = cr.parse_reset("You've hit your limit · resets 8pm (Asia/Tokyo)",
                                now.astimezone(NY))
...
class TestDatedResets(unittest.TestCase):
    def test_weekly_limit_with_a_date(self):
        ...
        got = cr.parse_reset("You've hit your weekly limit · resets Jul 22 at 6am (Europe/Warsaw)", now)
        self.assertAlmostEqual(hours(got), 58.633, places=2)

    def test_date_after_the_time(self):
        now = datetime(2026, 7, 19, 12, 0, tzinfo=UTC)
        got = cr.parse_reset("resets at 6am on Jul 22 (UTC)", now)
    ...
    def test_date_that_already_passed_this_year_rolls_to_next(self):
        ...
        got = cr.parse_reset("resets Jan 2 at 6am (UTC)", now)
        self.assertAlmostEqual(hours(got), 66.0, places=2)

    def test_tomorrow(self):
        now = datetime(2026, 7, 16, 20, 0, tzinfo=UTC)
        self.assertAlmostEqual(hours(cr.parse_reset("resets tomorrow at 9am (UTC)", now)), 13.0, places=2)

    def test_weekday(self):
        now = datetime(2026, 7, 16, 12, 0, tzinfo=UTC)   # a Thursday
        self.assertAlmostEqual(hours(cr.parse_reset("resets Saturday at 6am (UTC)", now)), 42.0, places=2)
    ...
    def test_variants(self):
        ...
        for text, want in cases.items():
            with self.subTest(text=text):
                self.assertAlmostEqual(hours(cr.parse_reset(text)), want, places=3)


class TestIso(unittest.TestCase):
    def test_iso_instant(self):
        now = datetime(2026, 7, 22, 0, 0, tzinfo=UTC)
        got = cr.parse_reset("resets at 2026-07-22T06:00:00Z", now)
...
class TestBoundaries(unittest.TestCase):
    def test_just_passed_reset_retries_immediately(self):
        ...
        self.assertEqual(cr.parse_reset("resets 10am (UTC)", now), 0.0)

    def test_long_past_reset_rolls_forward(self):
        now = datetime(2026, 7, 16, 20, 0, tzinfo=UTC)
        got = cr.parse_reset("resets 10am (UTC)", now)
        self.assertAlmostEqual(hours(got), 14.0, places=2)

    def test_nothing_parseable(self):
        self.assertIsNone(cr.parse_reset("You've hit your session limit"))
        self.assertIsNone(cr.parse_reset(""))
        self.assertIsNone(cr.parse_reset(None))

    def test_impossible_clock_is_rejected(self):
        self.assertIsNone(cr.parse_reset("resets 99:99"))
    ...
    def test_dst_spring_forward(self):
        ...
        got = cr.parse_reset("resets 4am (Europe/Warsaw)", now)
        self.assertAlmostEqual(hours(got), 2.0, places=2)
