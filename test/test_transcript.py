...
class TestRecordParsing(unittest.TestCase):
    ...
    def test_finds_the_limit_record(self):
        ...
        offset, found = cw.transcript_limit_records(self.path, 0)
    ...
    def test_offset_makes_reads_incremental(self):
        ...
        self.write(REAL)
        offset, found = cw.transcript_limit_records(self.path, offset)
        self.assertEqual(len(found), 1)
        # Reading again from the new offset must not re-report it.
        _, again = cw.transcript_limit_records(self.path, offset)
    ...
    def test_missing_file(self):
        offset, found = cw.transcript_limit_records(os.path.join(self.dir, "nope.jsonl"), 0)
        self.assertEqual((offset, found), (0, []))


class TestWatcher(unittest.TestCase):
    def setUp(self):
        self.dir = tempfile.mkdtemp(prefix="cw-w-")
        self.addCleanup(shutil.rmtree, self.dir, ignore_errors=True)
    ...
    def test_preexisting_content_is_not_replayed(self):
        ...
        w = cw.TranscriptWatcher(self.dir, poll=0)
    ...
    def test_poll_interval_is_respected(self):
        w = cw.TranscriptWatcher(self.dir, poll=100)
