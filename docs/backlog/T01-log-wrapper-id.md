...
All wrappers on the machine write to the same `~/.agent-retrier/log`, and a line
...
`~/.agent-retrier/log`, 2026-09-17 22:45–22:51: two `folding up` entries with
...
1. In `Logger.__call__` (agent-retrier.sh:3809), add a tag after the timestamp:
...
`Logger` (agent-retrier.sh:3800), `main()` — `start:` (3870) and
